...
import { inboxV2OutboxOutcomes } from "./repository-foundation";

/**
 * Independently purgeable terminal payload pointer. The immutable outcome keeps
 * only a boolean historical marker and safe result hash after this row expires.
 */
export const inboxV2OutboxTerminalPayloadRefs = pgTable(
  ...
  (table) => [
    primaryKey({
      name: "inbox_v2_outbox_terminal_payload_refs_pk",
      columns: [table.tenantId, table.intentId, table.outcomeRevision]
    }),
    foreignKey({
      name: "inbox_v2_outbox_terminal_payload_refs_outcome_fk",
      columns: [table.tenantId, table.intentId, table.outcomeRevision],
      foreignColumns: [
        inboxV2OutboxOutcomes.tenantId,
        inboxV2OutboxOutcomes.intentId,
        inboxV2OutboxOutcomes.outcomeRevision
      ]
    }).onDelete("cascade"),
    check(
      "inbox_v2_outbox_terminal_payload_refs_values_check",
      sql`${table.outcomeRevision} >= 1
        and public.inbox_v2_auth_payload_reference_safe(
          ${table.resultReference}, ${table.tenantId}
        )
        and isfinite(${table.recordedAt})`
    ),
    index("inbox_v2_outbox_terminal_payload_refs_recorded_idx").on(
      table.tenantId,
      table.recordedAt,
      table.intentId,
...
/** UPDATE is forbidden; retention-owned DELETE is the lifecycle purge boundary. */
export const INBOX_V2_OUTBOX_TERMINAL_PAYLOAD_REF_INTEGRITY_SQL = String.raw`
