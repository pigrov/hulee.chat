...
- Source integration foundation: contracts and persistence skeleton for source connections, source accounts, raw inbound events and normalized inbound events. Existing messenger channel connectors remain valid until the source pipeline is exercised by Public API, forms or email.
...
Inbox/conversation/message implementation for `MVP-COM-01`, `MVP-COM-02`,
`MVP-COM-03`, `MVP-UI-03` and related source/notification/reporting work is
planned and verified through `docs/product/inbox-v2-backlog.md`. These MVP rows
describe product outcomes; they must not be treated as a second task-status
tracker or completed from the current Inbox V1 prototype.

Inbox data classification, retention, subject export/delete, legal hold, audit
and backup-erasure behavior follows ADR 0015 and
`docs/product/inbox-v2-data-lifecycle-and-privacy.md`; an MVP shortcut may reduce
supported profiles but cannot use infinite retention, cascade deletion or copied
raw PII in generic audit/events.
...
| MVP-CON-02  | P0       | Contracts     | Module manifest and adapter contracts      | `ModuleManifest`, `AuthProvider`, `SourceAdapter`, `ChannelAdapter`, `StorageProvider`, `WebhookSink` contracts are exported and tested.                                                 |
...
| MVP-ID-02   | P0       | Permissions   | Minimal scoped RBAC                        | Versioned permissions plus server-derived tenant/resource scopes gate admin, inbox and API-key operations; role names are templates, not authority. Inbox V2 expansion follows ADR 0013. |
...
| MVP-MOD-03A | P0       | Modules       | Source integration foundation              | SourceConnection, SourceAccount, RawInboundEvent and NormalizedInboundEvent contracts and tables exist before non-messenger connectors.                                                  |
