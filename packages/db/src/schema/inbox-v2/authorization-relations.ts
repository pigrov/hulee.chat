...
/**
 * Durable provenance for an initial outbound dispatch created by the
 * authorized two-phase materialization coordinator. Standalone/legacy
 * dispatch writers intentionally do not create this row; the deferred domain
 * closure requires it only when an authorized mutation publishes the dispatch.
 */
export const inboxV2AtomicOutboundDispatchMaterializations = pgTable(
  ...
  (table) => [
    primaryKey({
      name: "inbox_v2_atomic_outbound_dispatch_materializations_pk",
      columns: [table.tenantId, table.dispatchId]
    }),
    foreignKey({
      name: "inbox_v2_atomic_outbound_dispatch_materializations_tenant_fk",
      columns: [table.tenantId],
      foreignColumns: [tenants.id]
    }).onDelete("cascade"),
    foreignKey({
      name: "inbox_v2_atomic_outbound_dispatch_materializations_stream_fk",
      columns: [
        table.tenantId,
        table.streamCommitId,
        table.mutationId,
    ...
    check(
      "inbox_v2_atomic_outbound_dispatch_materializations_values_check",
      sql`${table.streamPosition} >= 1
        and ${table.resultingRevision} = 1
        and isfinite(${table.createdAt})`
    ),
    index("inbox_v2_atomic_outbound_dispatch_materializations_commit_idx").on(
      table.tenantId,
      table.streamCommitId,
      table.mutationId,
      table.streamPosition,
...
/**
 * Commit-time provenance for the source-occurrence resolution side of one
 * source-originated Message creation. It prevents a resolution CAS/reference
 * from becoming visible without the exact SourceOccurrence stream change and
 * the Message produced by the same authorized mutation.
 */
export const inboxV2AtomicSourceResolutionMaterializations = pgTable(
  "inbox_v2_atomic_source_resolution_materializations",
  {
    tenantId: text("tenant_id").notNull(),
    sourceOccurrenceId: text("source_occurrence_id").notNull(),
    resolutionTransitionId: text("resolution_transition_id").notNull(),
  ...
  (table) => [
    primaryKey({
      name: "inbox_v2_atomic_source_resolution_materializations_pk",
      columns: [table.tenantId, table.sourceOccurrenceId]
    }),
    unique("inbox_v2_atomic_src_resolution_transition_uq").on(
      table.tenantId,
      table.resolutionTransitionId
    ),
    foreignKey({
      name: "inbox_v2_atomic_source_resolution_materializations_tenant_fk",
      columns: [table.tenantId],
      foreignColumns: [tenants.id]
    }).onDelete("cascade"),
    foreignKey({
      name: "inbox_v2_atomic_source_resolution_materializations_stream_fk",
      columns: [
        table.tenantId,
        table.streamCommitId,
        table.mutationId,
    ...
    check(
      "inbox_v2_atomic_source_resolution_materializations_values_check",
      sql`${table.streamPosition} >= 1
        and ${table.resultingRevision} >= 2
        and isfinite(${table.createdAt})`
    ),
    index("inbox_v2_atomic_source_resolution_materializations_commit_idx").on(
      table.tenantId,
      table.streamCommitId,
      table.mutationId,
      table.streamPosition,
    ...
    index("inbox_v2_atomic_source_resolution_materializations_message_idx").on(
      table.tenantId,
      table.messageId,
      table.streamPosition
    )
...
/**
 * DB-side equivalent of the atomic tenant-stream provider-I/O closure.
 * Authorization-relation commits keep their stricter provider-I/O ban;
 * only zero-authorization-delta domain commits use this function.
 */
export const INBOX_V2_AUTH_DOMAIN_PROVIDER_IO_CLOSURE_SQL = String.raw`
