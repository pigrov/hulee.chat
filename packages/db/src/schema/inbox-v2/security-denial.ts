import {
  ...
  inboxV2SecurityDenialActionSchema as securityDenialActionContract,
  inboxV2SecurityDenialAlertTypeSchema as securityDenialAlertTypeContract,
  inboxV2SecurityDenialDispositionSchema as securityDenialDispositionContract,
  inboxV2SecurityDenialKindSchema as securityDenialKindContract,
  inboxV2SecurityDenialPrincipalClassSchema as securityDenialPrincipalClassContract,
  inboxV2SecurityDenialPublicErrorClassSchema as securityDenialPublicErrorClassContract,
  inboxV2SecurityDenialReviewAggregationKindSchema as securityDenialReviewAggregationKindContract,
  inboxV2SecurityDenialReviewStatusSchema as securityDenialReviewStatusContract,
  inboxV2SecurityDenialReviewTypeSchema as securityDenialReviewTypeContract,
  inboxV2SecurityDenialReviewWriteSchema as securityDenialReviewWriteContract,
  inboxV2SecurityDenialRiskSchema as securityDenialRiskContract
...
const POLICY = INBOX_V2_SECURITY_DENIAL_POLICY;
const policyIdSql = sql.raw(`'${POLICY.policyId}'`);
const windowSecondsSql = sql.raw(String(POLICY.windowSeconds));
const retentionSecondsSql = sql.raw(String(POLICY.retentionSeconds));
const shardCountSql = sql.raw(String(POLICY.shardCount));
const detailBucketLimitSql = sql.raw(String(POLICY.detailBucketLimitPerShard));
const reviewCandidateLimitSql = sql.raw(
  String(POLICY.reviewCandidateLimitPerShard)
);

const reviewWriteDispositions =
  securityDenialReviewWriteContract.shape.disposition.options;

function nonEmptyEnumValues<Value extends string>(
  values: readonly Value[]
): [Value, ...Value[]] {
  const [first, ...rest] = values;
  if (first === undefined) {
    throw new Error("Inbox V2 security-denial enum cannot be empty.");
  }
  return [first, ...rest];
}

export const inboxV2SecurityDenialAction = pgEnum(
  "inbox_v2_security_denial_action",
  nonEmptyEnumValues(securityDenialActionContract.options)
);

export const inboxV2SecurityDenialPrincipalClass = pgEnum(
  "inbox_v2_security_denial_principal_class",
  nonEmptyEnumValues(securityDenialPrincipalClassContract.options)
);

export const inboxV2SecurityDenialKind = pgEnum(
  "inbox_v2_security_denial_kind",
  nonEmptyEnumValues(securityDenialKindContract.options)
);

export const inboxV2SecurityDenialPublicErrorClass = pgEnum(
  "inbox_v2_security_denial_public_error_class",
  nonEmptyEnumValues(securityDenialPublicErrorClassContract.options)
);

export const inboxV2SecurityDenialRisk = pgEnum(
  "inbox_v2_security_denial_risk",
  nonEmptyEnumValues(securityDenialRiskContract.options)
);

export const inboxV2SecurityDenialReviewType = pgEnum(
  "inbox_v2_security_denial_review_type",
  nonEmptyEnumValues(securityDenialReviewTypeContract.options)
);

export const inboxV2SecurityDenialAlertType = pgEnum(
  "inbox_v2_security_denial_alert_type",
  nonEmptyEnumValues(securityDenialAlertTypeContract.options)
);

export const inboxV2SecurityDenialDisposition = pgEnum(
  "inbox_v2_security_denial_disposition",
  nonEmptyEnumValues(securityDenialDispositionContract.options)
);

export const inboxV2SecurityDenialReviewDisposition = pgEnum(
  "inbox_v2_security_denial_review_disposition",
  nonEmptyEnumValues(reviewWriteDispositions)
);

export const inboxV2SecurityDenialReviewAggregationKind = pgEnum(
  "inbox_v2_security_denial_review_aggregation_kind",
  nonEmptyEnumValues(securityDenialReviewAggregationKindContract.options)
);

export const inboxV2SecurityDenialReviewStatus = pgEnum(
  "inbox_v2_security_denial_review_status",
  nonEmptyEnumValues(securityDenialReviewStatusContract.options)
);

/**
 * One fixed quota/serialization row per tenant, hour and deterministic shard.
 * This is deliberately independent from the Inbox tenant stream and outbox.
 */
export const inboxV2SecurityDenialWindowShards = pgTable(
  "inbox_v2_security_denial_window_shards",
  {
    ...
    shardNo: smallint("shard_no").notNull(),
    ...
    admittedDetailBucketCount: smallint(
      "admitted_detail_bucket_count"
    ).notNull(),
    admittedReviewCandidateCount: smallint(
      "admitted_review_candidate_count"
    ).notNull(),
    overflowCount: bigint("overflow_count", { mode: "bigint" }).notNull(),
    counterSaturated: boolean("counter_saturated").notNull(),
  ...
  (table) => [
    primaryKey({
      name: "inbox_v2_security_denial_window_shards_pk",
      columns: [table.tenantId, table.windowStartedAt, table.shardNo]
    }),
    foreignKey({
      name: "inbox_v2_security_denial_window_shards_tenant_fk",
      columns: [table.tenantId],
      foreignColumns: [tenants.id]
    }).onDelete("cascade"),
    check(
      "inbox_v2_security_denial_window_policy_check",
      sql`${table.policyId} = ${policyIdSql}
        and ${table.shardNo} between 0 and ${shardCountSql} - 1
        and ${table.windowEndedAt} =
          ${table.windowStartedAt} + make_interval(secs => ${windowSecondsSql})
        and ${table.expiresAt} =
          ${table.windowStartedAt} + make_interval(secs => ${retentionSecondsSql})`
    ),
    check(
      "inbox_v2_security_denial_window_counts_check",
      sql`${table.attemptCount} >= 1
        and ${table.admittedDetailBucketCount}
          between 0 and ${detailBucketLimitSql}
        and ${table.admittedReviewCandidateCount}
          between 0 and ${reviewCandidateLimitSql}
        and ${table.admittedDetailBucketCount} <= ${table.attemptCount}
        and ${table.admittedReviewCandidateCount} <=
          ${table.admittedDetailBucketCount}
        and ${table.overflowCount} between 0 and ${table.attemptCount}`
    ),
    check(
      "inbox_v2_security_denial_window_times_check",
      sql`isfinite(${table.windowStartedAt})
        and isfinite(${table.windowEndedAt})
        and isfinite(${table.firstSeenAt})
        and isfinite(${table.lastSeenAt})
        and isfinite(${table.expiresAt})
        and isfinite(${table.createdAt})
        and isfinite(${table.updatedAt})
        and ${table.firstSeenAt} >= ${table.windowStartedAt}
        and ${table.lastSeenAt} >= ${table.firstSeenAt}
        and ${table.lastSeenAt} < ${table.windowEndedAt}
        and ${table.updatedAt} >= ${table.createdAt}`
    ),
    index("inbox_v2_security_denial_window_expiry_idx").on(
      table.tenantId,
      table.expiresAt,
      table.windowStartedAt,
      table.shardNo
    ),
    index("inbox_v2_security_denial_window_activity_idx").on(
      table.tenantId,
...
/** At most sixteen redacted HMAC aggregates under one locked shard row. */
export const inboxV2SecurityDenialBuckets = pgTable(
  "inbox_v2_security_denial_buckets",
  {
    ...
    action: inboxV2SecurityDenialAction("action").notNull(),
    principalClass:
      inboxV2SecurityDenialPrincipalClass("principal_class").notNull(),
    fingerprintKeyEpoch: text("fingerprint_key_epoch").notNull(),
    actorFingerprint: text("actor_fingerprint").notNull(),
    denialKind: inboxV2SecurityDenialKind("denial_kind").notNull(),
    publicErrorClass:
      inboxV2SecurityDenialPublicErrorClass("public_error_class").notNull(),
    risk: inboxV2SecurityDenialRisk("risk").notNull(),
  ...
  (table) => [
    primaryKey({
      name: "inbox_v2_security_denial_buckets_pk",
      columns: [
        table.tenantId,
        table.windowStartedAt,
        table.shardNo,
        table.dedupeFingerprint
      ]
    }),
    foreignKey({
      name: "inbox_v2_security_denial_buckets_window_fk",
      columns: [table.tenantId, table.windowStartedAt, table.shardNo],
      foreignColumns: [
        inboxV2SecurityDenialWindowShards.tenantId,
        inboxV2SecurityDenialWindowShards.windowStartedAt,
        inboxV2SecurityDenialWindowShards.shardNo
      ]
    }).onDelete("cascade"),
    check(
      "inbox_v2_security_denial_buckets_identity_check",
      sql`${table.fingerprintKeyEpoch} ~
          '^security-denial-key:[a-f0-9]{16,32}$'
        and ${table.actorFingerprint} ~ '^hmac-sha256:[a-f0-9]{64}$'
        and ${table.dedupeFingerprint} ~ '^hmac-sha256:[a-f0-9]{64}$'
        and ${table.actorFingerprint} <> ${table.dedupeFingerprint}`
    ),
    check(
      "inbox_v2_security_denial_buckets_decision_check",
      sql`(${table.denialKind} not in (
          'unknown_or_hidden_resource', 'cross_tenant_probe'
        ) or ${table.publicErrorClass} = 'not_found')
        and (${table.denialKind} = 'manual_self_claim') =
          (${table.action} = 'identity.claim'
            and ${table.publicErrorClass} =
              'identity_claim_self_forbidden')
        and ${table.risk} = case
          when ${table.denialKind} = 'cross_tenant_probe'
            or ${table.action} = 'privacy.deletion.execute'
            then 'critical'::inbox_v2_security_denial_risk
          when ${table.denialKind} in (
              'manual_self_claim', 'unknown_or_hidden_resource'
            ) or ${table.action} in (
              'authorization.privileged_mutation', 'identity.claim'
            ) or ${table.action}::text like 'privacy.%'
            then 'high'::inbox_v2_security_denial_risk
          when ${table.denialKind} in ('state_guard', 'other_denied')
    ...
    check(
      ...
      sql`${table.shardNo} between 0 and ${shardCountSql} - 1
        and ${table.windowEndedAt} =
          ${table.windowStartedAt} + make_interval(secs => ${windowSecondsSql})
        and ${table.expiresAt} =
          ${table.windowStartedAt} + make_interval(secs => ${retentionSecondsSql})
        and ${table.occurrenceCount} >= 1`
    ),
    check(
      "inbox_v2_security_denial_buckets_times_check",
      sql`isfinite(${table.windowStartedAt})
        and isfinite(${table.windowEndedAt})
        and isfinite(${table.firstSeenAt})
        and isfinite(${table.lastSeenAt})
    ...
    index("inbox_v2_security_denial_buckets_action_idx").on(
      table.tenantId,
      table.action,
      table.lastSeenAt.desc(),
      table.dedupeFingerprint
    ),
    index("inbox_v2_security_denial_buckets_expiry_idx").on(
      table.tenantId,
      table.expiresAt,
      table.windowStartedAt,
      table.shardNo,
...
/**
 * Candidate rows are capped per shard; overflow rows are capped by the finite
 * review-type enum. Neither shape contains caller-selected target identifiers.
 */
export const inboxV2SecurityDenialReviewSignals = pgTable(
  "inbox_v2_security_denial_review_signals",
  {
    tenantId: text("tenant_id").notNull(),
    reviewSequence: bigint("review_sequence", { mode: "bigint" })
    ...
    shardNo: smallint("shard_no").notNull(),
    reviewType: inboxV2SecurityDenialReviewType("review_type").notNull(),
    aggregationKind:
      inboxV2SecurityDenialReviewAggregationKind("aggregation_kind").notNull(),
    candidateFingerprint: text("candidate_fingerprint"),
    aggregationKey: text("aggregation_key")
      .notNull()
      .generatedAlwaysAs(
        sql`case aggregation_kind
          when 'candidate' then candidate_fingerprint
          else 'overflow'
        end`
      ),
    alertType: inboxV2SecurityDenialAlertType("alert_type").notNull(),
    candidateRef: text("candidate_ref"),
    risk: inboxV2SecurityDenialRisk("risk").notNull(),
    status: inboxV2SecurityDenialReviewStatus("status").notNull(),
    triggerCount: bigint("trigger_count", { mode: "bigint" }).notNull(),
    windowEndedAt: timestamp("window_ended_at", {
      withTimezone: true,
      precision: 3
    }).notNull(),
  ...
  (table) => [
    primaryKey({
      name: "inbox_v2_security_denial_review_signals_pk",
      columns: [
        table.tenantId,
        table.windowStartedAt,
        table.shardNo,
    ...
    foreignKey({
      name: "inbox_v2_security_denial_review_signals_window_fk",
      columns: [table.tenantId, table.windowStartedAt, table.shardNo],
      foreignColumns: [
        inboxV2SecurityDenialWindowShards.tenantId,
        inboxV2SecurityDenialWindowShards.windowStartedAt,
    ...
    foreignKey({
      name: "inbox_v2_security_denial_review_signals_bucket_fk",
      columns: [
        table.tenantId,
        table.windowStartedAt,
        table.shardNo,
      ...
      foreignColumns: [
        inboxV2SecurityDenialBuckets.tenantId,
        inboxV2SecurityDenialBuckets.windowStartedAt,
        inboxV2SecurityDenialBuckets.shardNo,
        inboxV2SecurityDenialBuckets.dedupeFingerprint
      ]
    }).onDelete("cascade"),
    check(
      "inbox_v2_security_denial_review_shape_check",
      sql`(${table.aggregationKind} = 'candidate'
          and ${table.candidateFingerprint} ~
            '^hmac-sha256:[a-f0-9]{64}$'
          and (${table.candidateRef} is null
            or ${table.candidateRef} ~ '^internal-ref:[a-f0-9]{32,64}$')
          and (${table.reviewType} = 'manual_self_claim'
            or ${table.candidateRef} is null))
        or (${table.aggregationKind} = 'overflow'
          and ${table.candidateFingerprint} is null
          and ${table.candidateRef} is null)`
    ),
    check(
      "inbox_v2_security_denial_review_presentation_check",
      sql`case
          when ${table.reviewType} = 'cross_tenant_probe' then
            ${table.alertType} = 'security_probe_review'
              and ${table.risk} = 'critical'
          when ${table.reviewType} = 'guessed_identifier_probe' then
            ${table.alertType} = 'security_probe_review'
              and ${table.risk} = 'high'
          when ${table.reviewType} = 'manual_self_claim' then
            ${table.alertType} = 'identity_claim_review'
              and ${table.risk} = 'high'
          when ${table.reviewType} in (
            'denial_rate_exceeded', 'denial_volume_exceeded'
          ) then
            ${table.alertType} = 'abuse_threshold_alert'
              and ${table.risk} in ('high', 'critical')
          when ${table.reviewType} = 'destructive_execution_denied' then
            ${table.alertType} = 'privacy_control_review'
              and ${table.risk} = 'critical'
          else
            ${table.alertType} = 'privacy_control_review'
              and ${table.risk} in ('high', 'critical')
    ...
    check(
      ...
      sql`${table.shardNo} between 0 and ${shardCountSql} - 1
        ...
        and ${table.expiresAt} =
          ${table.windowStartedAt} + make_interval(secs => ${retentionSecondsSql})
        and ${table.triggerCount} >= 1`
    ),
    check(
      "inbox_v2_security_denial_review_times_check",
      sql`isfinite(${table.windowStartedAt})
        and isfinite(${table.windowEndedAt})
        and isfinite(${table.firstSeenAt})
        and isfinite(${table.lastSeenAt})
    ...
    index("inbox_v2_security_denial_review_status_idx").on(
      table.tenantId,
      table.status,
    ...
    index("inbox_v2_security_denial_review_sequence_idx").on(
      table.tenantId,
      table.reviewSequence.desc(),
      table.windowStartedAt.desc(),
      table.shardNo,
    ...
    index("inbox_v2_security_denial_review_type_sequence_idx").on(
      table.tenantId,
      table.reviewType,
      table.reviewSequence.desc(),
      table.windowStartedAt.desc(),
    ...
    index("inbox_v2_security_denial_review_status_type_sequence_idx").on(
      table.tenantId,
      table.status,
      table.reviewType,
      table.reviewSequence.desc(),
    ...
    index("inbox_v2_security_denial_review_expiry_idx").on(
      table.tenantId,
      table.expiresAt,
      table.windowStartedAt,
      table.shardNo,
...
/** Canonical bounded write/prune authority; intentionally no stream/outbox SQL. */
export const INBOX_V2_SECURITY_DENIAL_INTEGRITY_SQL = String.raw`
